"""3D Intelligence Globe — High-intensity indicators with robust data-driven hotspots."""
...
def render_map_panel() -> None:
    ...
    # Dynamic Hotspots from state
    # (Mapping internal cluster IDs to live data)
    hotspots = [
        {
            "lat": -34.60, "lng": -58.38, "size": 0.8, "color": "#fbbf24", "name": "BETA_CLUSTER",
            "cases": 3, "deaths": 1, "sentiment": "Concerned", "channel": "C5N",
            "article": "Ministry of Health monitors Ushuaia ports."
        },
        {
            "lat": -26.20, "lng": 28.04,  "size": 0.7, "color": "#fbbf24", "name": "ALPHA_CLUSTER",
            "cases": 2, "deaths": 1, "sentiment": "Alert", "channel": "SABC",
            "article": "NICD confirms Case 3 remains in isolated ICU."
        },
        {
            "lat": 14.93,  "lng": -23.51, "size": 1.4, "color": "#22c55e", "name": "MV_HONDIUS_POS",
            "cases": state.get("confirmed_cases", 5), "deaths": state.get("deaths", 3), 
            "sentiment": "Critical", "channel": "MARITIME",
            "article": f"Vessel Status: {state.get('ship_status', 'Active Signal')}."
        }
    ]
    ...
    <body>
      ...
      <script>
        ...
        const world = Globe()
          ...
          // 2. POINTS (High-Intensity 3D Pillars)
          .pointsData(hotspots)
          .pointLat('lat')
          .pointLng('lng')
          .pointColor('color')
          ...
          // 3. CUSTOM HTML BLINKING MARKERS
          ...
          .htmlElement(d => {{
            ...
            el.className = 'blink-light ' + (d.name.includes('HONDIUS') ? 'vessel-blink' : 'signal-blink');
            return el;
          }})
    ...
    st.markdown(
        "<div style='text-align:right; opacity:0.6;'><p style='color:#475569; font-size:0.5rem; font-family:monospace;'>ORBITAL_RECO_SYS v5.2 // HI_EMISSIVE: ON</p></div>",
        unsafe_allow_html=True
    )
